      ...
      <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
        {quickActionList.map(({ id, label, query, Icon }) => (
          <button
            ...
            onClick={() => onSelect(query)}
            disabled={disabled}
            className="group flex min-h-[56px] items-center gap-2.5 rounded-xl border border-[#A78BFA]/25 bg-[#21152F] p-3 text-left text-xs font-medium text-[#F7F7FA] shadow-xs transition-all hover:border-[#FF5C7A] hover:bg-[#2b1e3d] hover:shadow-[0_0_15px_-4px_rgba(255,92,122,0.3)] disabled:cursor-not-allowed disabled:opacity-50 sm:text-sm"
          >
            <span
              aria-hidden="true"
              className="grid size-8 shrink-0 place-items-center rounded-lg bg-[#0B0B12] text-[#FF5C7A] ring-1 ring-[#A78BFA]/30 transition-colors"
            >
              <Icon className="size-4 text-[#FF5C7A]" />
            </span>
            <span className="leading-tight text-[#AAA5B8] group-hover:text-[#F7F7FA]">{label}</span>
          </button>
        ))}
      </div>
    </div>
