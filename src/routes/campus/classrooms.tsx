...
export const Route = createFileRoute("/campus/classrooms")({
  head: () => ({
    meta: [
      { title: "Classrooms & Seminar Halls — SDIT Mangaluru" },
      {
        name: "description",
        content:
          "Explore the spacious, ICT-enabled classrooms, Wi-Fi connected lecture halls, modern seminar halls, and auditorium at Shree Devi Institute of Technology (SDIT), Kenjar.",
      },
      {
        property: "og:title",
        content: "Classrooms & Seminar Halls — SDIT Mangaluru",
      },
      {
        property: "og:description",
        content:
          "Spacious classrooms equipped with audio-visual presentation aids, Wi-Fi, and modern seminar facilities at SDIT.",
      },
    ],
  }),
  component: CampusClassroomsPage,
});

function CampusClassroomsPage() {
  const facility = campusFacilities.find((f) => f.id === "classrooms")!;
          ...
          <div className="mt-10 rounded-3xl border border-border bg-surface p-6 shadow-soft sm:p-8">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl font-bold text-foreground sm:text-2xl">
                Pedagogical &amp; Instructional Spaces
              </h2>
            </div>
                ...
                <div className="flex items-start justify-between gap-4 border-b border-border/60 pb-5">
                  <div className="flex items-center gap-3">
                    <span className="grid size-10 place-items-center rounded-xl bg-surface text-navy">
                      {idx === 0 && <Presentation className="size-5" />}
                      {idx === 1 && <Tv className="size-5" />}
                      {idx === 2 && <Building className="size-5" />}
                      {idx === 3 && <Radio className="size-5" />}
                    </span>
                    <div>
