...
export const Route = createFileRoute("/campus-life/events/samshodhan")({
  head: () => ({
    meta: [
      { title: "Samshodhan Project Exhibition — SDIT Events" },
      {
        name: "description",
        content:
          "Official Samshodhan technical project exhibition showcasing innovative engineering models and prototypes at SDIT Mangaluru.",
      },
      {
        property: "og:title",
        content: "Samshodhan Project Exhibition — SDIT Events",
      },
    ],
  }),
  component: SamshodhanPage,
});

export function SamshodhanPage() {
  const event = sditEvents.find((e) => e.slug === "samshodhan")!;
      ...
      <main className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <nav aria-label="Breadcrumb" className="mb-6 text-xs text-muted-foreground">
          <ol className="flex items-center gap-2">
            ...
            <li>/</li>
            <li className="font-semibold text-foreground">Samshodhan Project Exhibition</li>
          </ol>
        </nav>
        ...
        <div className="mt-12 grid gap-8 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-8">
            <div className="rounded-2xl border border-border bg-card p-6 shadow-soft sm:p-8">
              <h2 className="text-xl font-bold text-card-foreground">Exhibition Overview</h2>
              <p className="mt-4 text-sm leading-relaxed text-muted-foreground">
                {event.fullDescription}
              </p>

              <h3 className="mt-8 text-lg font-bold text-card-foreground">Key Highlights</h3>
